	...
	if (pManifest->LoadFromFile( filesystem, pszFile ))
	{
		FOR_EACH_SUBKEY( pManifest, pProtagKey )
		{
			...
			FOR_EACH_SUBKEY( pProtagKey, pSubKey )
			{
				...
				{
					pszSubKeyName += 3;
					if (!pszSubKeyName[0] || pszSubKeyName[0] == 's')
					{
						// Model
...
#ifdef EZ2
	// Legs
	if (pProtag.pszLegModel)
	{
		pSource->PrecacheModel( pProtag.pszLegModel );
	}
#endif
...
#ifdef EZ2
		if (pProtag.dictWpnData[i].pszDualVM)
		{
			pSource->PrecacheModel( pProtag.dictWpnData[i].pszDualVM );
		}
#endif
	}
...
#ifdef EZ2
		if (pProtag.pszLegModel)
			Msg( "\t\t\t\tLeg model: \"%s\" (%i, %i)\n", pProtag.pszLegModel, pProtag.nLegSkin, pProtag.nLegBody );
#endif
...
#ifdef EZ2
			if (pProtag.dictWpnData[j].pszDualVM)
				Msg( "\t\t\t\tDual Viewmodel: \"%s\"\n", pProtag.dictWpnData[j].pszDualVM );
#endif
